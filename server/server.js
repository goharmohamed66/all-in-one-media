...
function saveConfig() {
  try {
    ...
    // contains the Anthropic API key — never world-readable
...
// only the app's own origin may connect — '*' let any open website listen in
const io = new Server(server, { cors: { origin: [`http://127.0.0.1:${PORT}`, `http://localhost:${PORT}`] } });

app.use(express.json({ limit: '25mb' })); // large enough for base64 image uploads
app.use(express.static(PUBLIC_DIR));

function emit(socketId, event, payload) {
  // never broadcast: an event without a target goes nowhere, not to everyone
...
function tikwmSerializedRequest(endpoint, form) {
  ...
  // The tail stored in _tikwmChain must NEVER be a rejected promise — one
  // failed request used to poison every later TikTok call until app restart.
  const task = _tikwmChain.then(async () => {
...
function igRefreshCookies(setCookie) {
  ...
  // session cookies — atomic replace (temp + rename) with owner-only perms
  try {
    const tmp = file + '.tmp';
...
async function igKeywordSearch(query, maxResults, seen, onCard) {
  ...
  // budget is per-keyword: with the shared cross-keyword dedupe set, comparing
  // seen.size directly meant keyword #2 onwards never issued a single request
  while (seen.size - startCount < maxResults && guard < 12) {
    guard++;
...
async function igSearchStream(keywords, perKeyword, socketId, runId = 0) {
  const seen = new Set();
  let idx = 0;
  let err = null;
  for (const kw of keywords) {
    ...
    if (!raw) continue;
    const emitNew = (card) => {
      if (card.id && !seen.has(card.id)) { seen.add(card.id); emit(socketId, 'list:item', { card, index: idx++, runId }); }
    };
    try {
      if (raw.startsWith('#')) {
        // explicit hashtag → hashtag search
        const tag = raw.replace(/^#/, '').replace(/\s+/g, '').trim();
        emit(socketId, 'list:status', { message: `بحث: #${tag}`, runId });
        await igHashtagSearch(tag, perKeyword, emitNew);
      } else {
        // free-text keyword/phrase → keyword search (like the mobile app)
        emit(socketId, 'list:status', { message: `بحث: ${raw}`, runId });
  ...
  if (seen.size === 0) {
    if (err && (err.code === 'NO_IG_LOGIN' || err.code === 'IG_LOGIN_EXPIRED')) {
      emit(socketId, 'list:error', { message: 'سجّل دخول إنستجرام من الإعدادات أولاً (أو الجلسة انتهت — أعد تسجيل الدخول).', runId });
    } else {
      emit(socketId, 'list:error', { message: (err && err.message) || 'لا توجد نتائج لهذه الكلمة.', runId });
    }
  } else {
    emit(socketId, 'list:done', { count: seen.size, runId });
  }
}
...
function ytdlpFlatList(targetUrl, { onItem, sleepRequests = '0.5', timeoutMs = 600000, useCookies = false, platform = null } = {}) {
  return new Promise((resolve, reject) => {
    ...
    proc.on('close', (code) => {
      ...
      if (code !== 0) return reject(new Error(friendlyYtdlpError(stderr, code, platform || detectPlatform(targetUrl))));
      resolve(items);
    });
  });
}
...
function ytdlpSingleInfo(url, { timeoutMs = 90000, useCookies = false, platform = null } = {}) {
  return new Promise((resolve, reject) => {
    ...
    proc.on('close', (code) => {
      clearTimeout(timer);
      if (code !== 0 || !out.trim()) return reject(new Error(friendlyYtdlpError(err, code, platform || detectPlatform(url))));
...
function friendlyYtdlpError(stderr, code, platform = null) {
  ...
  if (/login required|private|sign in|cookies|empty media/.test(s)) {
    // without the platform arg this ignored the in-app login file entirely
    return hasCookies(platform)
      ? 'هذا المحتوى خاص أو يتطلب تسجيل دخول (تأكد أن جلسة الدخول ما زالت سارية).'
      : 'هذا المحتوى خاص أو يتطلب تسجيل دخول. سجّل الدخول من داخل البرنامج أو فعّل "الكوكيز من المتصفح".';
  }
...
function cacheSet(key, data) {
  ...
  // bounded: evict the oldest entries so a long session can't grow forever
  if (_cache.size > 100) {
    const oldest = [..._cache.entries()].sort((a, b) => a[1].ts - b[1].ts).slice(0, _cache.size - 100);
...
function getAnthropicClient() {
  ...
  // default SDK timeout is 10 minutes with retries — a network partition would
  // stall a verify run for ~30 min; 2 minutes per call is plenty for images
  return new Anthropic({ apiKey: CONFIG.anthropicApiKey, timeout: 120000, maxRetries: 1 });
}

// The app relies on structured outputs (JSON-schema responses) — models without
// that capability 400 on every AI call, so they must not appear in the picker.
function modelSupportsApp(m) {
  return !!(m && m.capabilities && m.capabilities.structured_outputs
...
app.post('/api/ai/connect', async (req, res) => {
  ...
  try {
    ...
    for await (const m of client.models.list()) {
      if (/claude/i.test(m.id) && modelSupportsApp(m)) models.push({ id: m.id, name: m.display_name || m.id });
    }
...
app.post('/api/ai/model', async (req, res) => {
  ...
  if (client) {
    try {
      ...
      if (!modelSupportsApp(m)) return res.status(400).json({ error: 'هذا الموديل لا يدعم مزايا البرنامج.' });
    } catch { return res.status(400).json({ error: 'موديل غير معروف.' }); }
  }
...
app.post('/api/ai/keywords', async (req, res) => {
  ...
  try {
    // no explicit thinking config: newer models think inside the same max_tokens
    // budget (and some reject a fixed setting), so give headroom instead
    ...
    if (r.stop_reason === 'refusal') {
      return res.status(400).json({ error: 'الموديل رفض تحليل هذه الصورة — جرّب صورة أو موديل آخر.' });
    }
...
function extractFrames(videoPath, outDir, n, durationSec) {
  return new Promise((resolve, reject) => {
    ...
    // stderr MUST be drained: on corrupt videos ffmpeg spams decode errors and
    // blocks forever once the 64KB pipe buffer fills
    proc.stdout.resume();
    proc.stderr.resume();
    const killer = setTimeout(() => { try { proc.kill('SIGKILL'); } catch {} }, 120000);
...
async function claudeVerdicts(client, model, contentBlocks, expectedIds) {
  ...
  if (r.stop_reason === 'refusal') {
    // treated like malformed output: every expected id comes back 'unsure'
    return parseVerdictResults('', expectedIds);
  }
  const textBlock = (r.content || []).find((b) => b.type === 'text');
...
// workRoot is unique per verify run so concurrent runs can't rm each other's files.
async function deepVerifyOne(client, model, productImgs, item, workRoot) {
  const workDir = path.join(workRoot, String(item.id));
  try {
    let play = '';
...
// a revoked/invalid key must abort the whole run, not grind through doomed calls
function isAuthError(e) {
  return !!(e && (e.status === 401 || e.status === 403));
}

async function runVerify(client, model, productImgs, items, socketId, deep, deepOnly = false, runId = 0) {
  const verdicts = new Map();
  const workRoot = path.join(os.tmpdir(), 'mediagrab-verify',
    `run_${Date.now()}_${Math.floor(Math.random() * 1e6)}`);
  const report = (item, v, stage) => {
    verdicts.set(item.id, v.verdict);
    emit(socketId, 'verify:item', { id: item.rawId || item.id, verdict: v.verdict, reason: v.reason || '', stage, runId });
  };
  ...
  for (const batch of deepOnly ? [] : verifyChunk(items, VERIFY_BATCH)) {
    ...
    try {
      ...
      if (isAuthError(e)) throw new Error('مفتاح Claude مرفوض — راجع المفتاح في الإعدادات.');
  ...
  if (deep) {
    ...
    for (const item of unsure) {
      emit(socketId, 'verify:item', { id: item.rawId || item.id, verdict: 'checking', stage: 'deep', runId });
      try {
        report(item, await deepVerifyOne(client, model, productImgs, item, workRoot), 'deep');
      } catch (e) {
        if (isAuthError(e)) throw new Error('مفتاح Claude مرفوض — راجع المفتاح في الإعدادات.');
  ...
  emit(socketId, 'verify:done', { counts, runId });
}
...
app.post('/api/ai/verify', (req, res) => {
  ...
  const { cards, socketId, deep = true, deepOnly = false, runId = 0 } = req.body || {};
  ...
  // hard deadline: whatever happens, the UI must get a terminal event
  const deadline = new Promise((_, rej) => {
    const t = setTimeout(() => rej(new Error('انتهت مهلة التحقق — حاول مرة أخرى.')), 12 * 60 * 1000);
    if (t.unref) t.unref();
  });
  Promise.race([
    runVerify(client, model, productImgs, items, socketId, deep === false ? false : true, deepOnly === true, runId),
    deadline,
  ]).catch((e) => emit(socketId, 'verify:error', { message: e.message || 'فشل التحقق.', runId }));
});
...
app.post('/api/info', async (req, res) => {
  const { url, socketId, runId = 0 } = req.body || {};
  ...
  try {
    if (isList) {
      ...
      streamList(url, platform, socketId, runId);
      return;
    }
    ...
    const msg = e.message === 'NOT_A_VIDEO' ? 'هذا منشور صور وليس فيديو.' : (e.message || 'فشل جلب المعلومات.');
    return res.status(500).json({ error: msg });
  }
});

async function streamList(url, platform, socketId, runId = 0) {
  ...
  const run = (useCookies) => ytdlpFlatList(url, {
    ...
    useCookies,
    platform,
    onItem: (card, i) => emit(socketId, 'list:item', { card, index: i, runId }),
  });
  try {
    const items = await runWithCookieFallback(run, platform);
    cacheSet(cacheKey, items);
    emit(socketId, 'list:done', { count: items.length, runId });
  } catch (e) {
    const stale = cacheGet(cacheKey);
    if (stale) {
      stale.data.forEach((card, i) => emit(socketId, 'list:item', { card, index: i, runId }));
      emit(socketId, 'list:done', { count: stale.data.length, stale: true, runId });
    } else {
      emit(socketId, 'list:error', { message: e.message || 'فشل جلب القائمة.', runId });
    }
  }
}
...
async function tiktokSearchStream(keywords, perKeyword, socketId, runId = 0) {
  const seen = new Set();
  let idx = 0;
  let errored = null;
  for (const kw0 of keywords) {
    const kw = normalizeArabic(kw0) || kw0;
    emit(socketId, 'list:status', { message: `بحث: ${kw0}`, runId });
    let cursor = 0;
    let pulled = 0;
    let guard = 0;
    try {
      while (pulled < perKeyword && guard < 12) {
        ...
        for (const v of videos) {
          ...
          seen.add(card.id);
          emit(socketId, 'list:item', { card, index: idx++, runId });
        }
  ...
  if (seen.size === 0 && errored) emit(socketId, 'list:error', { message: errored.message || 'فشل البحث.', runId });
  else emit(socketId, 'list:done', { count: seen.size, runId });
}

// YouTube multi-keyword search (yt-dlp ytsearch) + cross-keyword de-dup, streamed live.
async function youtubeSearchStream(keywords, perKeyword, socketId, runId = 0) {
  const seen = new Set();
  let idx = 0;
  let errored = null;
  for (const kw0 of keywords) {
...
app.post('/api/search', async (req, res) => {
  const { platform, query, keywords, socketId, runId = 0 } = req.body || {};
  ...
  try {
    if (platform === 'tiktok') {
      ...
      tiktokSearchStream(kws, perKeyword, socketId, runId);
      return;
    }

    if (platform === 'youtube') {
      res.json({ streaming: true, keywords: kws });
      youtubeSearchStream(kws, perKeyword, socketId, runId);
      return;
    }
    ...
    if (platform === 'instagram') {
      ...
      if (/^https?:/i.test(q) || q.startsWith('@')) {
        ...
        streamList(target, 'instagram', socketId, runId);
        return;
      }
      res.json({ streaming: true, keywords: kws });
      igSearchStream(kws, perKeyword, socketId, runId);
      return;
    }
    if (platform === 'facebook') {
      ...
      res.json({ streaming: true });
      streamList(target, 'facebook', socketId, runId);
      return;
    }
...
app.post('/api/download', async (req, res) => {
  ...
  const id = `${Date.now()}_${++_downloadSeq}`; // monotonic — same-millisecond requests must not collide
  res.json({ id });
...
function downloadFile(url, destNoExt, id, socketId, redirects = 0) {
  return new Promise((resolve, reject) => {
    ...
    const req = lib.get(
      u,
      // timeout = no data for 60s → abort; a stalled CDN must not hang forever
      { headers: { 'User-Agent': YTDLP_UA, 'Referer': 'https://www.tiktok.com/' }, timeout: 60000 },
      (res) => {
        if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
          res.resume();
        ...
        res.pipe(file);
        res.on('aborted', () => { file.destroy(); fs.rm(dest, { force: true }, () => {}); reject(new Error('انقطع التنزيل.')); });
        file.on('finish', () => file.close(() => resolve(dest)));
        file.on('error', (e) => { fs.rm(dest, { force: true }, () => {}); reject(e); });
      }
    );
    req.on('timeout', () => req.destroy(new Error('انتهت مهلة التنزيل — الرابط بطيء أو منتهي.')));
    req.on('error', reject);
  });
}
...
async function downloadTikTok(item, format, id, socketId) {
  ...
  if (format === 'mp3') {
    ...
    try {
      ...
      // never leave the intermediate _tmp video in the user's Downloads
...
function downloadViaYtdlp(item, format, id, socketId, useCookies = false) {
  return new Promise((resolve, reject) => {
    ...
    proc.on('close', (code) => {
      ...
      if (code !== 0) {
        const err = new Error(friendlyYtdlpError(stderr, code, platform));
        err.raw = stderr;
        return reject(err);
      }
...
// ───────────────────────────── MEDIA PROXY ─────────────────────────────
// Strict registrable-domain allowlist (exact host or subdomain — never substring).
const PROXY_ALLOW = [
  'tiktok.com', 'tiktokcdn.com', 'tiktokcdn-us.com', 'tiktokcdn-eu.com', 'tiktokv.com',
  'muscdn.com', 'byteoversea.com', 'ibyteimg.com',
  'instagram.com', 'cdninstagram.com', 'fbcdn.net', 'facebook.com',
  'ytimg.com', 'googlevideo.com', 'ggpht.com', 'youtube.com',
  'tikwm.com', 'akamaized.net',
];

function refererForHost(host) {
  if (/tiktok|muscdn|byteoversea|ibyteimg|tikwm/.test(host)) return 'https://www.tiktok.com/';
  if (/instagram/.test(host)) return 'https://www.instagram.com/';
...
app.get('/api/proxy/media', (req, res) => {
  ...
  if (!isAllowedHost(target.hostname, PROXY_ALLOW)) {
    return res.status(403).send('host not allowed');
  }

  let upstream = null;
  res.on('close', () => { if (upstream) upstream.destroy(); });

  // every hop (including redirect targets) re-validates scheme + allowlist
  const doRequest = (urlObj, redirects) => {
    if (redirects > 6) { if (!res.headersSent) res.status(502).send('too many redirects'); return; }
    if (urlObj.protocol !== 'http:' && urlObj.protocol !== 'https:') {
      if (!res.headersSent) res.status(403).send('bad redirect scheme');
      return;
    }
    if (!isAllowedHost(urlObj.hostname, PROXY_ALLOW)) {
      if (!res.headersSent) res.status(403).send('redirect host not allowed');
      return;
    }
    const lib = urlObj.protocol === 'http:' ? http : https;
    const headers = { 'User-Agent': YTDLP_UA, 'Accept': '*/*' };
    const ref = refererForHost(urlObj.hostname.toLowerCase());
    if (ref) headers.Referer = ref;
    if (req.headers.range) headers.Range = req.headers.range;

    upstream = lib.get(urlObj, { headers, timeout: 30000 }, (r) => {
      if (r.statusCode >= 300 && r.statusCode < 400 && r.headers.location) {
        r.resume();
        let next;
        try { next = new URL(r.headers.location, urlObj); } catch {
...
// unknown API routes are a bug in the caller — surface them instead of
// returning index.html (which made every typo'd endpoint an opaque JSON error)
app.all('/api/*', (req, res) => {
  res.status(404).json({ error: `مسار غير معروف: ${req.path}` });
});
...
// one stray rejection must not kill the fork (it used to take the whole app down)
process.on('unhandledRejection', (e) => {
  console.error('[server] unhandled rejection:', (e && e.message) || e);
});
process.on('uncaughtException', (e) => {
  console.error('[server] uncaught exception:', (e && e.stack) || e);
});

server.on('error', (e) => {
  if (e && e.code === 'EADDRINUSE') {
    console.error(`[server] port ${PORT} already in use — another instance is running.`);
    process.exit(90); // distinct code so main.js can explain instead of hanging
  }
